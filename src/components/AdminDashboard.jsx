            ...
            <div className="space-y-4 animate-in fade-in">
              ...
              <div className="bg-white border border-slate-100 rounded-xl overflow-hidden divide-y divide-slate-50 shadow-sm">
                {homeBlocks.map((block, index) => (
                  <div key={block.id} className="p-4 hover:bg-slate-50/50 transition-colors group">
                    <div className="flex items-center gap-3 mb-4">
                      ...
                      <span className="text-[7px] font-black bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded uppercase">{block.type}</span>
                      <span className="text-[10px] font-bold text-slate-400 italic">ID: {block.id}</span>
                    </div>

                    <div className="space-y-4 p-5 bg-slate-50 rounded-[2rem] border border-slate-100">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="text-[9px] font-black uppercase text-slate-400 ml-3 mb-1 block tracking-widest">Section Title</label>
                          <input
                            defaultValue={block.title || ''}
                            onBlur={(e) => updateField('homeBlocks', block.id, { title: e.target.value })}
                            className="w-full p-4 rounded-2xl border-none shadow-sm text-sm font-bold text-emerald-900 outline-none focus:ring-2 ring-emerald-500/20"
                          />
                        </div>
                        <div>
                          <label className="text-[9px] font-black uppercase text-slate-400 ml-3 mb-1 block tracking-widest">Smart Link (Destination)</label>
                          <input
                            defaultValue={block.linkTo || ''}
                            placeholder="e.g. program:Banquet or vision"
                            onBlur={(e) => updateField('homeBlocks', block.id, { linkTo: e.target.value })}
                            className="w-full p-4 rounded-2xl border-none shadow-sm text-sm font-mono text-emerald-700 bg-white outline-none focus:ring-2 ring-emerald-500/20"
                          />
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="text-[9px] font-black uppercase text-slate-400 ml-3 mb-1 block tracking-widest">Image URL</label>
                          <input
                            defaultValue={block.imageUrl || ''}
                            onBlur={(e) => updateField('homeBlocks', block.id, { imageUrl: e.target.value })}
                            className="w-full p-4 rounded-2xl border-none shadow-sm text-sm outline-none"
                          />
                        </div>
                        <div>
                          <label className="text-[9px] font-black uppercase text-slate-400 ml-3 mb-1 block tracking-widest">Button Label</label>
                          <input
                            defaultValue={block.ctaText || ''}
                            onBlur={(e) => updateField('homeBlocks', block.id, { ctaText: e.target.value })}
                            className="w-full p-4 rounded-2xl border-none shadow-sm text-xs font-black uppercase tracking-tighter text-emerald-800"
                          />
                        </div>
                      </div>

                      <div>
                        <label className="text-[9px] font-black uppercase text-slate-400 ml-3 mb-1 block tracking-widest">Body Content</label>
                        <textarea
                          defaultValue={block.content || ''}
                          rows={2}
                          onBlur={(e) => updateField('homeBlocks', block.id, { content: e.target.value })}
                          className="w-full p-4 rounded-2xl border-none shadow-sm text-sm outline-none resize-none"
                        />
                      </div>
                      ...
                      <div className="flex justify-end gap-2 pt-2">
                        <button
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-white rounded-full text-[9px] font-black uppercase text-slate-400 shadow-sm"
                        >
                          <Link size={12} /> Copy ID Link
                        </button>
                        <button
                          onClick={() => removeItem('homeBlocks', block.id, 'block')}
                          className="px-4 py-2 bg-red-50 text-red-500 rounded-full text-[9px] font-black uppercase"
                        >
                          Delete
                        </button>
                      </div>
                    ...
                    {block.type === 'grid' && (
                      <div className="mt-6 space-y-4 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                        <div className="flex justify-between items-center mb-2">
                          <span className="text-[10px] font-black uppercase text-emerald-800 tracking-widest">Grid Items</span>
                          <button
                            onClick={() => {
                              const newItems = [...(block.items || []), { id: Date.now(), title: '', desc: '', imageUrl: '', ctaText: '', ctaLink: '' }];
                              updateField('homeBlocks', block.id, { items: newItems });
                            }}
                            className="text-[9px] bg-emerald-600 text-white px-4 py-1.5 rounded-full font-black uppercase"
                          >+ Add Card</button>
                        </div>
                        {block.items?.map((item, idx) => (
                          <div key={item.id || idx} className="p-4 bg-slate-50 rounded-2xl border border-slate-100 space-y-3">
                            <div className="flex gap-2">
                              <input
                                className="flex-1 text-[11px] font-bold text-emerald-900 bg-white p-3 rounded-xl outline-none"
                                placeholder="Card Title"
                                defaultValue={item.title}
                                onBlur={(e) => {
                                  const newItems = block.items.map((it, i) => i === idx ? { ...it, title: e.target.value } : it);
                                  updateField('homeBlocks', block.id, { items: newItems });
                                }}
                              />
                              <button onClick={() => {
                            ...
                            <input
                              className="w-full text-[10px] text-slate-500 bg-white p-3 rounded-xl outline-none"
                              placeholder="Image URL"
                              defaultValue={item.imageUrl}
                              onBlur={(e) => {
                                const newItems = block.items.map((it, i) => i === idx ? { ...it, imageUrl: e.target.value } : it);
                                updateField('homeBlocks', block.id, { items: newItems });
                              }}
                            />
                            <div className="grid grid-cols-2 gap-2">
                              <input
                                className="text-[10px] font-black uppercase text-emerald-800 bg-white p-3 rounded-xl outline-none"
                                placeholder="Button Text"
                                defaultValue={item.ctaText}
                                onBlur={(e) => {
                                  const newItems = block.items.map((it, i) => i === idx ? { ...it, ctaText: e.target.value } : it);
                                  updateField('homeBlocks', block.id, { items: newItems });
                                }}
                              />
                              <input
                                className="text-[10px] font-mono text-emerald-600 bg-white p-3 rounded-xl outline-none"
                                placeholder="Smart Link"
                                defaultValue={item.ctaLink}
                                onBlur={(e) => {
                                  const newItems = block.items.map((it, i) => i === idx ? { ...it, ctaLink: e.target.value } : it);
                                  updateField('homeBlocks', block.id, { items: newItems });
                                }}
                              />
                              <select
                                className="text-[10px] font-black uppercase bg-white p-3 rounded-xl outline-none shadow-sm cursor-pointer border border-transparent focus:border-emerald-200"
                                // 1. Force the dropdown to show exactly what is in the database
                                value={item.ctaStyle || 'solid'}
                                onChange={(e) => {
                                  const selectedStyle = e.target.value;

                                  // 2. Meticulously map through the items to find the ONE being edited
                                  const updatedItems = block.items.map((it) => {
                                    // Use the unique ID (or index) to identify the card
                                    if (it.id === item.id) {
                                      return { ...it, ctaStyle: selectedStyle };
                                    }
                                    return it;
                                  });

                                  // 3. Push the entire updated array back to the block
                                  updateField('homeBlocks', block.id, { items: updatedItems });
                                }}
                              >
                    ...
                    {block.type === 'slideshow' && (
                      <div className="mt-6 space-y-4 bg-white p-6 rounded-[2rem] border border-slate-100 shadow-sm">
                        <div className="flex justify-between items-center">
                          <span className="text-[10px] font-black uppercase text-emerald-800 tracking-widest">Slides</span>
                          <button
                            onClick={() => updateField('homeBlocks', block.id, { slides: [...(block.slides || []), { url: '', link: '' }] })}
                            className="text-[9px] bg-emerald-600 text-white px-4 py-1.5 rounded-full font-black uppercase"
                          >+ Add Slide</button>
                        </div>
                        {block.slides?.map((slide, i) => (
                          <div key={i} className="flex flex-col gap-2 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                            <input className="text-[11px] bg-white p-3 rounded-xl outline-none" placeholder="Image URL" defaultValue={slide.url} onBlur={(e) => { let s = [...block.slides]; s[i].url = e.target.value; updateField('homeBlocks', block.id, { slides: s }); }} />
                            <div className="flex gap-2">
                              <input className="flex-1 text-[11px] font-mono text-emerald-600 bg-white p-3 rounded-xl outline-none" placeholder="Smart Link" defaultValue={slide.link} onBlur={(e) => { let s = [...block.slides]; s[i].link = e.target.value; updateField('homeBlocks', block.id, { slides: s }); }} />
                              <button onClick={() => { let s = block.slides.filter((_, idx) => idx !== i); updateField('homeBlocks', block.id, { slides: s }); }} className="text-red-400 px-2 font-bold">×</button>
                            </div>
                          </div>
                        ))}
                      </div>
            ...
            <div className="space-y-12 animate-in fade-in">
              <div className="space-y-6">
                <div className="border-b pb-4">
                  ...
                  <p className="text-[9px] font-bold text-slate-300 uppercase tracking-widest mt-1">Foundation Architect</p>
                </div>

                {/* THE 4 PILLARS GRID */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-10 gap-y-8">
                  {[1, 2, 3, 4].map((num) => (
                    <div key={num} className="space-y-2 group bg-slate-50/50 p-4 rounded-2xl relative">
                      <div className="flex justify-between items-center">
                        <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Pillar {num} Title</label>

                        {/* PILLAR SMART LINK BUTTON */}
                        <button
                          onClick={() => {
                            const link = `${window.location.origin}/?page=vision&id=pillar-${num}`;
                            navigator.clipboard.writeText(link);
                            alert(`Smart Link for Pillar ${num} copied!`);
                          }}
                          className="p-1 text-slate-300 hover:text-emerald-600 transition-colors"
                          title="Copy Pillar Link"
                        >
                          <Link size={12} />
                        </button>
                      </div>
                      ...
                      <input
                        className="w-full bg-transparent border-b border-slate-100 p-0 py-1 text-xs font-bold text-emerald-900 outline-none focus:border-emerald-900 transition-all"
                        defaultValue={siteContent?.[`pillar${num}Title`]}
                        onBlur={(e) => updateSite({ [`pillar${num}Title`]: e.target.value })}
                      />

                      {/* Link & CTA for Pillars */}
                      <div className="grid grid-cols-2 gap-2 mt-2">
                        <input
                          className="w-full bg-white border border-slate-100 rounded p-1.5 text-[9px] font-mono text-emerald-600"
                          placeholder="Target Link"
                          defaultValue={siteContent?.[`pillar${num}Link`]}
                          onBlur={(e) => updateSite({ [`pillar${num}Link`]: e.target.value })}
                        />
                        <input
                          className="w-full bg-white border border-slate-100 rounded p-1.5 text-[9px] font-black uppercase"
                          placeholder="Button Label"
                          defaultValue={siteContent?.[`pillar${num}Cta`]}
                          onBlur={(e) => updateSite({ [`pillar${num}Cta`]: e.target.value })}
                        />
                      </div>

                      <textarea
                        className="w-full bg-slate-50/50 border border-slate-100 rounded-lg p-2 text-[10px] h-16 resize-none outline-none focus:bg-white"
                        defaultValue={siteContent?.[`pillar${num}Desc`]}
                        onBlur={(e) => updateSite({ [`pillar${num}Desc`]: e.target.value })}
                      />
                    </div>
                  ))}
                </div>
              ...
              {/* THE VISION ACTS LIST */}
              <div className="space-y-6 pt-8 border-t border-slate-100">
                <div className="flex justify-between items-center border-b pb-4">
                  <h3 className="font-bold text-emerald-900 text-[10px] uppercase tracking-widest">Vision Acts</h3>
                  <button onClick={() => addItem('visionActs', { title: '', desc: '', link: '' })} className="text-emerald-700 font-bold text-[9px] bg-emerald-50 px-4 py-1.5 rounded-full shadow-sm">+ ADD ACT</button>
                </div>
                  ...
                  {visionActs.map(act => (
                    <div key={act.id} className="relative group pb-4 border-b border-slate-50">
                      <div className="flex justify-between items-start mb-1">
                        <input
                          className="font-bold text-emerald-900 text-[11px] w-full bg-transparent border-none p-0 outline-none uppercase"
                          defaultValue={act.title}
                          onBlur={(e) => updateField('visionActs', act.id, { title: e.target.value })}
                        />
                        <div className="flex items-center gap-2">
                          {/* ACT SMART LINK BUTTON */}
                          <button
                            onClick={() => {
                              const link = `${window.location.origin}/?page=vision&id=${act.id}`;
                              navigator.clipboard.writeText(link);
                              alert(`Smart Link for "${act.title || 'Act'}" copied!`);
                            }}
                            className="p-1 text-slate-300 hover:text-emerald-600 transition-colors"
                          >
                            <Link size={12} />
                          </button>
                          <button onClick={() => removeItem('visionActs', act.id, 'act')} className="text-red-200 hover:text-red-400">×</button>
                        </div>
                      </div>

                      <input
                        className="w-full text-[9px] font-mono text-emerald-600 bg-emerald-50/50 px-2 py-1 rounded mb-2 outline-none border border-transparent focus:border-emerald-200"
                        placeholder="Target Link (e.g. /register)"
                        defaultValue={act.link}
                        onBlur={(e) => updateField('visionActs', act.id, { link: e.target.value })}
                      />

                      <textarea
                        className="w-full text-[10px] bg-transparent p-0 border-none outline-none h-16 resize-none leading-relaxed"
                        defaultValue={act.desc}
                        onBlur={(e) => updateField('visionActs', act.id, { desc: e.target.value })}
                      />
                    </div>
                  ))}
                </div>
