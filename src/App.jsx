...
export default function ChurchPortal() {
  ...
  const handleTabChange = (tabName) => {
    ...
    // Check if there's a specific form requested in the URL
    const urlParams = new URLSearchParams(window.location.search);
    const idParam = urlParams.get('id');

    // ONLY pick the first form if there is NO ID in the URL
    ...
    // Fixed: window.scrollTo was inside the function, and the extra }; was here
...
useEffect(() => {
  ...
  if (activeTab === 'register' && forms.length > 0 && !idParam) {
    const firstVisibleForm = forms.find(f => f.isVisible) || forms[0];
    if (firstVisibleForm) {
      setActiveFormId(firstVisibleForm.id);
    }
...
useEffect(() => {
  ...
  if (pageParam) {
    ...
    // 1. Logic for Registration Forms
    ...
    if (pageParam === 'home' && idParam) {
      ...
      }, 800); // Increased slightly for a smoother landing
    }
  }
